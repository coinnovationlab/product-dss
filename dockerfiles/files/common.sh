...
xml_append() {
  ...
  pos=1
  first_attr=$5
  [[ ${property_xpath} =~ ^(.*)(/_?:?${property_name}).*$ ]]
  parent_xpath=${BASH_REMATCH[1]}
  parent_pname=${BASH_REMATCH[2]}
  #echo $parent_pname
  [[ ${first_attr} =~ ^(.*)=(.*) ]]
  first_attr_name=${BASH_REMATCH[1]}
  first_attr_value=${BASH_REMATCH[2]}
  #echo $property_xpath
  cmd=(ed -L -O -a "${property_xpath}" -t 'elem' -n "${property_name}" -v "${property_value}" "${target_file}")
  xmlstarlet "${cmd[@]}"
  for i in "${@:5}"; do
    [[ $i =~ ^(.*)=(.*) ]]
    attr_name=${BASH_REMATCH[1]}
    ...
    #echo "DEBUG $attr_value"
    if  [[ ${pos} -eq 1 ]]; then
      echo "p_path = ${parent_xpath} p_name = ${property_name} attr_name = ${attr_name} attr_value = ${attr_value}"
      cmd=(ed -L -O -i "${parent_xpath}/${parent_pname}[not(@${attr_name})]" -t "attr" -n "${attr_name}" -v "${attr_value}" "${target_file}")
      xmlstarlet "${cmd[@]}"
      #echo "${parent_xpath}/${property_name}[not(@${attr_name})]"
    else
      cmd=(ed -L -O -i "${parent_xpath}/${parent_pname}[@${old_attr_name}=\"${old_attr_value}\"]" -t "attr" -n "${attr_name}" -v "${attr_value}" "${target_file}")
      xmlstarlet "${cmd[@]}"
      #echo "${parent_xpath}/${property_name}[@${old_attr_name}=\"${old_attr_value}\"]"
    fi
    old_attr_name=${attr_name}
...
xml_append_attr_dss() {
  ...
  attr_name=${BASH_REMATCH[1]}
  attr_value=${BASH_REMATCH[2]}
  cmd=(ed -L -O -i "${property_xpath}/_:${property_name}" -t "attr" -n "${attr_name}" -v "${attr_value}" "${target_file}")
  xmlstarlet "${cmd[@]}"
}
